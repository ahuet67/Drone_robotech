...
/**
 * Created by Naif on 02/11/2016.
 */
public class MyJoystickView extends SurfaceView implements SurfaceHolder.Callback, View.OnTouchListener {
    ...
    private MyJoystickListener joystickCallBack;




    public interface MyJoystickListener {
        void onJoystickMoved(float xPourcent,float yPourcent,int id);
    }
    //Constructeurs
    public MyJoystickView(Context c) {
        super(c);
        getHolder().addCallback(this);
        setOnTouchListener(this);
        if(c instanceof MyJoystickView.MyJoystickListener) //verifie que l'on a bien implementé
            joystickCallBack = (MyJoystickListener) c;
    }
    public MyJoystickView(Context c,AttributeSet a) {
        super(c,a);
        getHolder().addCallback(this);
        setOnTouchListener(this);
        if(c instanceof MyJoystickView.MyJoystickListener) //verifie que l'on a bien implementé
    ...
    public MyJoystickView(Context c, AttributeSet a,int style) {
        super(c,a,style);
        getHolder().addCallback(this);
        setOnTouchListener(this);
        if(c instanceof MyJoystickView.MyJoystickListener) //verifie que l'on a bien implementé
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        setupDimensions();
        drawJoystick(getBottomRectX()+getLargeurRect()/2,getBottomRectY());
        ...
        if(v.equals(this)) {
            if(event.getAction() != event.ACTION_UP) {
                ...
                if(event.getY() >= deplacementYhaut && event.getY() <= deplacementYbas && event.getX() >= deplacementXgauche
                    ...
                    joystickCallBack.onJoystickMoved(((event.getX()-getWidth()/4)*100)/((3*getWidth()/4)-(getWidth()/4)),(event.getY()-getBottomRectY())*100/(getTopRectY()-getBottomRectY()),getId());
